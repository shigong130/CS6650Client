...
public class Assignment {
    ...
    public static void main(String[] args){
        ...
        String ec2="http://52.23.184.215:8080/SkiResortsServer_war/";
        ...
        //Assignment obj = new Assignment(256, 20000, 20, 40, ec2);
        Assignment obj = new Assignment(arg0, arg1, arg2, arg3, ec2);
        obj.runAssignment();
    }
    ...
    public Assignment(int maxNumThreads, int numSkiers, int numRuns, int numLifts, String serverAddress) {
        if(maxNumThreads>256 || numSkiers>50000 || numRuns>20 || numLifts>60 || numLifts<5){
    ...
    public void runAssignment(){
        ...
        logger.info("Throughput: " + (double)httpRecordFullList.size()/(tEnd-tStart));
    ...
    private List<Thread> generateThreads(List<int[]> idRangeList, int numberOfRequest, int[] timeRange,
                                               int[] liftIdRange, String phaseName, CountDownLatch latch){
        List<Thread> list = new ArrayList<Thread>();
        for(int i=0; i<idRangeList.size(); i++){
            String name = phaseName+"_"+i;
            Thread threads = new Thread(new Worker(
                name, numberOfRequest, idRangeList.get(i)[0], idRangeList.get(i)[1], timeRange[0], timeRange[1],
                    ...
                    httpRecordFullList, serverAddress
            ));
            list.add(threads);
        }
